...
class TensorTrain(LowRankTensor):
    def __init__(self, dims, max_rank,learned_scale=False, **kwargs):
        ...
        self.build_prior() #comment it if parameters do not match
    ...
    def build_prior(self):
        self.lamb = []
        for r in self.max_ranks[1:-1]:
            self.lamb.append(self.add_variable(torch.ones(r,1)/np.sqrt(r)))

    def compute_loglikelihood(self):
        log_loss = 0
        l = len(self.factors)
        for i in range(1,l-1):
            U = torch.moveaxis(self.factors[i],0,1)
            var = torch.sqrt(self.lamb[i-1]@self.lamb[i].T)


            log_loss += -1/2*torch.sum((U/var)**2)
            log_loss -= torch.sum(torch.log(var))
        
        U = torch.squeeze(self.factors[0])
        var = torch.squeeze(self.lamb[0])
        log_loss += -1/2*torch.sum((U/var)**2)
        log_loss -= torch.sum(torch.log(var))

        U = torch.squeeze(self.factors[-1])
        var = self.lamb[-1]
        log_loss += -1/2*torch.sum((U/var)**2)
        log_loss -= torch.sum(torch.log(var))

        for lamb in self.lamb:
            log_loss += torch.sum(-5*lamb)
        return log_loss
    ...
    def _nn_init(self):
        ...
        factor_stddev = torch.pow(
            torch.pow(torch.tensor(1.0 * self.max_rank),torch.tensor( -self.order + 1)) * torch.square(self.target_stddev), 1 / (2.0 * self.order))
    ...
    def _build_factors(self):
        ...
        for i in range(len(self.factors)):
            self.factors[i] = torch.nan_to_num(self.factors[i],nan=0,posinf=1, neginf=-1)
    ...
    def estimate_rank(self, threshold=1e-4):
        out = []
        for x in self.rank_parameters:
            v,i = torch.kthvalue(x[:],max(1,int(x.shape[0]*49/50)))
            v = v.item()
            tol = min(v,threshold)
            out.append(int(sum(x > tol)))
        return out

    def prune_ranks(self, threshold=1e-2):
        ...
        for x in self.rank_parameters:
            v,i = torch.kthvalue(-x[:],max(1,int(x.shape[0]*1/50)))
            v = -v.item()
            tol = min(v,threshold)
            # tol = max(tol,threshold/100)
            y = torch.nn.functional.threshold(x,tol,0)
            # y[y>0] = y[y>0] + tol
            self.masks.append(y)
        # self.masks = self.rank_parameters

    def get_kl_divergence_to_prior(self):
        kl = 0
        for x in self.rank_parameters:
            kl += torch.sum(torch.abs(x))
        return kl
        # kl = 0
        # for U,p in zip(self.factors[:-1],self.rank_parameters):
        #     p = torch.unsqueeze(p**2,0)
        #     p = torch.unsqueeze(p,0)
        #     kl += torch.sum((U)**2/p)/2
        #     kl += torch.sum(torch.log(p))*(U.shape[0]*U.shape[1])/2
        #     kl += 2*torch.sum(p)
...
class TensorTrainMatrix(LowRankTensor):
    ...
    def build_prior(self):
        ...
        # self.lamb = []
        # for r in self.max_ranks[1:-1]:
        #     self.lamb.append(torch.nn.Parameter(torch.ones(r,1)/np.sqrt(r)))
    ...
    def compute_loglikelihood(self):
        ...
        l = len(self.factors)
        for i in range(1,l-1):
            U = torch.moveaxis(self.factors[i],-1,1)
            var = torch.sqrt(self.lamb[i-1]@self.lamb[i].T)
            log_loss += -1/2*torch.sum((U/var)**2)
            log_loss -= torch.sum(torch.log(var))
    ...
    def full_from_factors(self,factors,quant=None):
...
#            factors = [x*y for x,y in zip(factors,self.masks)]+[self.masks[-1].view([-1,1,1,1])*factors[-1]]
        ...
        for i in range(1, num_dims):
            ...
            if quant!=None:
                res = quant(torch.matmul(res, curr_core))  
            else:
