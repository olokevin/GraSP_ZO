...
class TensorizedLinear_module_tonn(nn.Module):
    def __init__(self,
        ...
        elif type(max_rank)==list:
            if len(max_rank) == self.order+1:
                self.tt_rank = max_rank
            else:
                raise (ValueError("Rank is a list, but length does not match"))
        else:
            raise (ValueError("Rank is neither a list nor int"))
    ...
    def forward(self, x, ranks = None,scale = None,reconstruct = False,rank_update=False,return_factors = False,prune=False):
    # def forward(self, x):
        x_shape = tuple(x.shape)
        batch_sz = x_shape[0:-1]
        ...
        # permute batch_sz back (batch is not only at dim 0)
        t_dim = len(batch_sz)-1
        output = torch.unsqueeze(output, dim=t_dim)
        output = torch.transpose(output, t_dim, -1)
...
class TensorizedEmbeddingBag(nn.Module):
    def __init__(self,
                 init=None,
                 shape=None,
                 tensor_type='TensorTrainMatrix',
        ...
        super(TensorizedEmbeddingBag,self).__init__()
...
#        rows = gather_rows(self.tensor, x_ind)
        ...
        # print(rows[:,0:2,0:2])
        if self.mode == 'mean':
            return torch.mean(rows.to(x.device),dim=1)
        elif self.mode == 'sum':
            return torch.sum(rows.to(x.device),dim=1)


class TensorizedEmbedding_byone_Bag(nn.Module):
    def __init__(self,
                 init=None,
                 shape=None,
                 tensor_type='TensorTrainMatrix',
        ...
        super(TensorizedEmbedding_byone_Bag,self).__init__()
    ...
    def forward(self, x,rank_update=True):
        shape = self.shape[0]
        if len(x.shape)==1:
            out_tensor = torch.zeros(x.shape[0],np.prod(self.shape[1])).to(x.device)
            for i in range(x.shape[0]):
                ind_l = x[i]
                ind_c = np.unravel_index(ind_l.cpu(),shape)

                out = self.tensor.factors[0][:,ind_c[0],:,:]
                for j in range(1,len(shape)):
                    out = torch.tensordot(out,self.tensor.factors[j][:,ind_c[j],:,:],dims=[[-1],[0]])
                out = torch.flatten(out)
                out_tensor[i,:] = out
        
        elif len(x.shape)==2:
            out_tensor = torch.zeros(x.shape[0],x.shape[1],np.prod(self.shape[1])).to(x.device)
            for i in range(x.shape[0]):
                for k in range(x.shape[1]):
                    ind_l = x[i,k]
                    ind_c = np.unravel_index(ind_l,shape)
                    ...
                    out_tensor[i,k,:] = out
        if self.mode == 'mean':
            return torch.mean(out_tensor,dim=1)
        elif self.mode == 'sum':
            return torch.sum(out_tensor,dim=1)
...
class TTM_forward(torch.autograd.Function):
    @staticmethod
